...
// check for row elements by left to right ---->
for (y <- 0 until grid.size) {
  for (x <- 0 until grid.size - elements + 1) {
    var product = 1
    for (n <- 0 until elements) {
      product = product * grid(y)(x + n)
    }
    if (product > max) {
      max = product
    }
...
// check for column elements by top to down V V V V
for (y <- 0 until grid.size - elements + 1) {
  for (x <- 0 until grid.size) {
    var product = 1
    for (n <- 0 until elements) {
      product = product * grid(y + n)(x)
    }
    if (product > max) {
      max = product
    }
...
// check for diagonally elements left top to right bottom \\\\\\
for (y <- 0 until grid.size - elements + 1) {
  for (x <- 0 until grid.size - elements + 1) {
    var product = 1
    for (n <- 0 until elements) {
      product = product * grid(y + n)(x + n)
    }
    if (product > max) {
      max = product
    }
...
// check for diagonally elements left top to right bottom but reverse direction /////
for (y <- 0 until grid.size - elements + 1) {
  for (x <- elements - 1 until grid.size) {
    var product = 1
    for (n <- 0 until elements) {
      product = product * grid(y + n)(x - n)
    }
